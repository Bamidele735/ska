...
export const getReservesForPair = async () => {
  ...
  try {
    ...
    reserves.push(web3.utils.fromWei(result._reserve1)); // reserve[1] should be ASTRO
    return {
      success: true,
      reserves // reserve 0: ASTRO, 1: USDC
    }
  } catch (error) {
...
export const getAstroAllowance = async () => {
  const web3 = store.getState().auth.web3;
  if (!web3) return { success: false }
  try {
    ...
    let ret_allowance = await AstroContract.methods.allowance(accounts[0], JOEAddress).call();
    ...
    console.log('[Astro Allowance] = ', error);
    return {
      success: false,
      result: "Something went wrong "
    }
...
export const approveAstro = async () => {
  const web3 = store.getState().auth.web3;
  if (!web3) return { success: false }
  try {
    const accounts = await web3.eth.getAccounts();
    ...
    const AstroContract = new web3.eth.Contract(AstroABI, AstroAddress);
    await AstroContract.methods.approve(JOEAddress, maxAmount).send({ from: accounts[0] });
    return {
      success: true
    }
  } catch (error) {
    console.log('[Approve Astro] = ', error);
    return {
      success: false,
      result: "Something went wrong "
    }
