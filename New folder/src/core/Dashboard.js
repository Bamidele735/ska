...
export const onClaimAstro = async (claimAstro) => {
  const web3 = store.getState().auth.web3;
  if (!web3) return { success: false }
  try {
    const accounts = await web3.eth.getAccounts();
    ...
    const claimAmount = web3.utils.toWei(claimAstro.toString());
    const AstroContract = new web3.eth.Contract(AstroABI, AstroAddress);
    await AstroContract.methods.approve(DashboardAddress, claimAmount).send({ from: accounts[0] });
